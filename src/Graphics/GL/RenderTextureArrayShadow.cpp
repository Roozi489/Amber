#include "Graphics/GL/RenderTextureArrayShadow.h"
...
void RenderTextureArrayShadow::create(int w, int h, TextureWrapMode wrap)
{
	...
	glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, toUnderlyingType(TextureInternalFormat::R32F), width, height, 6, 0, toUnderlyingType(TextureFormat::Red), GL_FLOAT, nullptr);
	...
	glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, toUnderlyingType(TextureInternalFormat::Depth32F), width, height, 6, 0, toUnderlyingType(TextureFormat::DepthComponent), GL_FLOAT, nullptr);
	glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
