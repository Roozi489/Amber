...
void VulkanRenderer::init()
{
	...
	pickPhysicalDevice();
	createLogicalDevice();
	createSwapchain(g_window.getWidth(), g_window.getHeight());
	createImageViews();
	createRenderPass();
...
void VulkanRenderer::handleWindowSizeChange(int newWidth, int newHeight)
{
	recreateSwapchain(newWidth, newHeight);
}

void VulkanRenderer::renderAll(Time delta)
{
	updateUniformBuffer();
}

void VulkanRenderer::finalRenderToMainFramebuffer()
{
	uint32 imageIndex;
	...
	{
		recreateSwapchain(outWidth, outHeight);
		return;
	}
...
{
	auto checkLayers = [](const auto& checkNames, const std::vector<vk::LayerProperties>& layers)
	{
...
void VulkanRenderer::createSurface()
{
...
#if defined(VK_USE_PLATFORM_WIN32_KHR)
		{
			auto const createInfo = vk::Win32SurfaceCreateInfoKHR()
				.setHinstance(g_window.getHInstance())
				.setHwnd(g_window.getHWnd());
...
#else
#error "Non supported platform. Only Windows is supported"
#endif
	}
...
void VulkanRenderer::createSwapchain(int width, int heigth)
{
	outWidth = width;
	outHeight = heigth;
	...
	if (surfCapabilities.currentExtent.width == std::numeric_limits<uint32_t>::max())
	{
		...
		swapchainExtent.height = heigth;
	}
	else
	{
		// If the surface size is defined, the swap chain size must match
		swapchainExtent = surfCapabilities.currentExtent;
		outWidth = surfCapabilities.currentExtent.width;
		outHeight = surfCapabilities.currentExtent.height;
	}
	...
	// If existing swapchain was re-created destroy the old swapchain
...
void VulkanRenderer::createCommandBuffers()
{
	...
	for (size_t i = 0; i < swapchainBuffers.size(); i++)
	{
		...
		vk::RenderPassBeginInfo renderPassBeginInfo = vk::RenderPassBeginInfo()
			...
			.setRenderArea(vk::Rect2D(vk::Offset2D(0, 0), vk::Extent2D(static_cast<uint32>(outWidth), static_cast<uint32>(outHeight))))
...
void VulkanRenderer::createImage(uint32 width, uint32 height, vk::Format format, vk::ImageTiling tiling, vk::ImageUsageFlags usage, vk::MemoryPropertyFlags properties, vk::Image& image, vk::DeviceMemory& imageMemory)
{
...
void VulkanRenderer::createImageView(vk::Image image, vk::Format format, vk::ImageAspectFlags aspectFlags, vk::ImageView& imageView)
{
...
void VulkanRenderer::copyImage(vk::Image srcImage, vk::Image dstImage, uint32 width, uint32 height)
{
	vk::CommandBuffer commandBuffer = beginSingleTimeCommands();
...
uint32 VulkanRenderer::findMemoryType(uint32 typeFilter, vk::MemoryPropertyFlags properties)
{
...
void VulkanRenderer::recreateSwapchain(int newWidth, int newHeight)
{
	...
	createSwapchain(outWidth, outHeight);
	createImageViews();
	createRenderPass();
	createGraphicsPipeline();
	createDepthResources();
