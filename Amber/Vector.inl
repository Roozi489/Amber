...
template <typename T>
Vector3<T> Vector3<T>::operator+(const Vector3& rhs) { return Vector3(x + rhs.x, y + rhs.y, z + rhs.z); }
...
template <typename T>
Vector3<T> Vector3<T>::operator*(T rhs) { return Vector3(x * rhs, y * rhs, z * rhs); }
...
{
    return Vector2(x, y) / (vec.magnitude() * vec.magnitude());
}
